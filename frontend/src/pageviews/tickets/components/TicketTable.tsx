    ...
    <div className="surface-card overflow-x-auto">
      <table className="data-table">
        ...
        <tbody>
          {tickets.map(ticket => (
            <tr key={ticket.id}>
              <td><p className="font-semibold">{ticket.codigo}</p><p className="mt-1 font-mono text-xs" style={{ color: 'var(--muted)' }}>{ticket.id.slice(0, 8)}</p></td>
              ...
              <td>
                <div className="flex justify-end gap-2">
                  <button onClick={() => inspect(ticket)} className="grid size-9 place-items-center rounded-lg border" aria-label="Ver detalle"><Eye size={16} /></button>
                  {operational && ticket.estado === 'ACTIVO' && <>
                    <button onClick={() => pay(ticket)} className="rounded-lg bg-emerald-600 px-3 text-xs font-semibold text-white">Cobrar</button>
                    <button onClick={() => cancel(ticket)} className="grid size-9 place-items-center rounded-lg border text-red-500" aria-label="Cancelar ticket"><XCircle size={16} /></button>
                  </>}
                </div>
              </td>
            </tr>
